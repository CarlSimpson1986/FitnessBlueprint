-- ============================================================================
-- 0030_programme_end_date.sql
-- The 6-week programmes (membership_plans.programme_length_days = 42)
-- never ended: book_session() only looked for an active membership, so a
-- member could keep booking on a 6-week programme indefinitely. Full
-- function body copied forward from 0025 (never edit a past migration in
-- place) with one addition: reject sessions dated after the programme's
-- last day. The daily reminders cron also marks finished programmes
-- 'expired' so they stop showing as active.
-- ============================================================================
...
begin
  ...
  -- New in 0030: a fixed-length programme (the 6-week programmes, 42
